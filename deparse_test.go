...
func TestDeparse(t *testing.T) {
    for idx, test := range parseTests {
        //fmt.Println("testing tree", test.inputTree, "\n", test.expectedParsed, "\n\n")
        actualDeparsed := test.inputTree.Deparse()
        if actualDeparsed != test.expectedParsed {
            t.Errorf("%d failed. Deparse Tree(%s)\nExpected: %s (length: %d)\nActual:   %s (length: %d)\n\n",
                idx,
                test.inputTree,
                test.expectedParsed,
                len(test.expectedParsed),
                actualDeparsed,
                len(actualDeparsed))
        } else {
            t.Log(idx, "ok")
